...
export const CustomActionModal: React.FC<{}> = ({
 ...
 const {custom_action_open,set_custom_action_open} = useContext(ModeContext);
 const {client,user, selected_iot_server} = useContext(MainContext);
  return (
    <Modal isOpen={custom_action_open} onRequestClose={() => {set_custom_action_open(false)}}>
      <Formik
        initialValues={{
          name: "",
          action_name:""
        ...
        validate={()=>{}}
        onSubmit={(data)=>{
            client!!.send("request_hoi_action",{server_id:selected_iot_server, bot_name:data.name, action:data.action_name});
        ...
        {({ isSubmitting }) => (
          <Form className={`flex-col w-full`}>
            ...
            <div className={`flex pt-2 items-center`}>
              ...
              <ButtonLink type="button" onClick={()=>{set_custom_action_open(false)}}>
                {"Cancel"}
              </ButtonLink>
            </div>
          </Form>
