...
export const ConnectModal: React.FC<{}> = ({
 ...
 const {integration_add_open,set_integration_add} = useContext(ModeContext);
 const {client,user} = useContext(MainContext);
  return (
    <Modal isOpen={integration_add_open} onRequestClose={() => {set_integration_add(false)}}>
      <Formik
        initialValues={{
        ...
        {({ isSubmitting }) => (
          <Form className={`flex-col w-full`}>
            ...
            <div className={`flex pt-2 items-center`}>
              ...
              <ButtonLink type="button" onClick={()=>{set_integration_add(false)}}>
                {"Cancel"}
              </ButtonLink>
            </div>
          </Form>
